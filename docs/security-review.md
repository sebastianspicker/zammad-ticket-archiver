...
| 2.2 | **M2 – Ingest payload schema validation** | Dev | Define a minimal Pydantic model for the webhook body (e.g. require `ticket` or `ticket_id`; allow additional fields). Use it in the ingest route so that invalid structure returns 400 before scheduling background work. Keep HMAC as the authentication mechanism; schema is a sanity check and blast-radius reduction. | Test: Valid payload 202; payload missing ticket id 400; oversized or deeply nested payload rejected by body limit or schema. |
